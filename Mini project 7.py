...
def open_attendance_calculator():
    ...
    result_label = ctk.CTkLabel(window, text="", font=("Segoe UI", 12), text_color="#1e90ff", wraplength=300, justify="left")
    ...
    def calculate_attendance_plan():
        try:
            ...
            total_bunkable_classes = f"{bunkable_classes * weeks_left:.0f}"
...
def open_todo_list():
    ...
    task_listbox = tk.Listbox(todo_window, font=("Segoe UI", 12), width=35, height=10)
...
def open_gpa_calculator():
    ...
    gpa_window.geometry("350x500")

    def label(text):
        ctk.CTkLabel(gpa_window, text=text, font=("Segoe UI", 12), text_color="#1e90ff").pack(pady=5)

    label("Subject Name")
    subject_entry = ctk.CTkEntry(gpa_window, font=("Segoe UI", 12))
    subject_entry.pack()

    label("ISA 1 Score (out of 40)")
    isa1_entry = ctk.CTkEntry(gpa_window, font=("Segoe UI", 12))
    isa1_entry.pack()

    label("ISA 2 Score (out of 40)")
    isa2_entry = ctk.CTkEntry(gpa_window, font=("Segoe UI", 12))
    isa2_entry.pack()

    label("Assignment Score (out of 10)")
    assignment_entry = ctk.CTkEntry(gpa_window, font=("Segoe UI", 12))
    assignment_entry.pack()

    label("ESA Score (out of 100)")
    esa_entry = ctk.CTkEntry(gpa_window, font=("Segoe UI", 12))
    esa_entry.pack()

    label("Lab Score (only for Chemistry/Python/Physics)")
    lab_entry = ctk.CTkEntry(gpa_window, font=("Segoe UI", 12))
    lab_entry.pack()

    result_label = ctk.CTkLabel(gpa_window, text="", font=("Segoe UI", 12), wraplength=300, justify="left", text_color="#1e90ff")
    result_label.pack(pady=20)
...
title = tk.Label(root, text="Student Toolkit", font=("Segoe UI", 18, "bold"), bg="#0a0a0a", fg="#1e90ff")
title.pack(pady=40)
